...
namespace Day19CharpDepthFirst;
class Program
{
...
// Determine the quality level of each blueprint using the largest number of geodes it could produce in 24 minutes. What do you get if you add up the quality level of all of the blueprints in your list?
    ...
    {
        List<BluePrint> blueprints = GetBluePrints("data.txt");
        ...
        for (int i=1; i<=3; i++)
        {
            int maxGeode = MaxGeodes(blueprints[i-1], 32, resources);
            Console.WriteLine(maxGeode);
            puzzleAnswer *= maxGeode;
        }

        stopwatch.Stop();
        Console.WriteLine("Puzzle Answer: {0}", puzzleAnswer);
        Console.WriteLine("Time elapsed: {0}", stopwatch.Elapsed, "s");
    }
    ...
    public static int MaxGeodes(BluePrint bluePrint, int minutes, Dictionary<string, int> resources)
    {
        ...
        // Perform depth-first search to explore the space of possible action sequences
        while (queue.Count > 0)
        {
            var state = queue.Pop();
            ...
            if (state.minutes > 0)
            {
                //check if pointless resources
                HashSet<string> excessResources = new HashSet<string>();
                foreach (string resource in resourceNames.GetRange(0, 3))
                {
                    if (state.resources[resource] > bluePrint.maxSpend[resource]*state.minutes)
                    {
                        excessResources.Add(resource);
                    }
                }
                foreach (string resource in state.robots.Keys)
                {
                    if (state.robots[resource] > bluePrint.maxSpend[resource])
                    {
                        excessResources.Add(resource);
                    }
                }
                State mineState = CreateMineState(state, resourceNames);

                //add geode robot if can afford
                        ...
                        state.resources["obsidian"] >= bluePrint.robotCosts["geode"]["obsidian"])
                {
                    ...
                    var newState = CreateNewState(mineState, bluePrint, "geode");
                    queue.Push(newState);
                }
                else
                {
                    //if can afford a robot, add it to the queue
                    ...
                    foreach (string excessResource in excessResources)
                    {
                        options.Remove(excessResource);
                    }
                    foreach (string dontBuy in state.dontBuyNext)
                    {
                        options.Remove(dontBuy);
                    }
                    foreach (string option in options)
                    {
                        //add newState to queue
                        queue.Push(CreateNewState(mineState, bluePrint, option));
                    }
                    foreach (string option in options)
                    {
                        mineState.dontBuyNext.Add(option);
                    }

                    queue.Push(mineState);
                }
            }
    ...
    static State CreateNewState(State mineState, BluePrint bluePrint, string option)
    {
        //create a deep copy of state
        var newState = new State(mineState.resources, mineState.robots, mineState.minutes);
        newState.dontBuyNext = new List<string>();
        //subtract cost of robot from resources
        newState.resources = new Dictionary<string, int>(){
            {"ore", mineState.resources["ore"] - bluePrint.robotCosts[option]["ore"]},
            {"clay", mineState.resources["clay"] - bluePrint.robotCosts[option]["clay"]},
            {"obsidian", mineState.resources["obsidian"] - bluePrint.robotCosts[option]["obsidian"]},
    ...
    static State CreateMineState(State state, List<string> resourceNames)
    {
        //create a deep copy of state.resources
        var mineState = new State(new Dictionary<string, int>(){
        ...
        mineState.dontBuyNext = new List<string>();
        foreach (string dontBuy in state.dontBuyNext)
        {
            mineState.dontBuyNext.Add(dontBuy);
        }
...
class BluePrint
{
    ...
    public Dictionary<string, int> maxSpend = new Dictionary<string, int>(){
            {"ore", 0},
            {"clay", 0},
            {"obsidian", 0},
            {"geode", 0}
    ...
    public BluePrint(string rawData)
    {
        ...
        {
            //find maximum cost of each resource
            foreach (var cost in robotCosts)
            {
                if (cost.Value[resource] > maxSpend[resource])
                {
                    maxSpend[resource] = cost.Value[resource];
                }
            }
        }
    }
...
class State
{
    public List<string> dontBuyNext = new List<string>();
    public int minutes;
